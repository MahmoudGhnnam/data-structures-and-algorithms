...
const createList = (availableItems) => {
  ...
  availableItems.forEach(element => {
    if (element.available) { array.push(element.name) }
  });
  return array;
};
...
const fizzbuzz = (arr) => {
  ...
  arr.forEach((element, index) => {
    if (element % 3 === 0) { arr[index] = 'Fizz' }
    else if (element % 5 === 0) { arr[index] = 'Buzz' }
    if ((element % 3 === 0) && (element % 5 === 0)) { arr[index] = 'Fizz Buzz' }
  });
  return arr;
};
