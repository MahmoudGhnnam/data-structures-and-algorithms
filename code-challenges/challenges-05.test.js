...
const templateWithJQuery = () => {
  ...
  starWarsPeople.forEach((item) => {
    ...
    $('main').append(newTemp);
    counter++;
    $(`section:nth-child(${counter}) h2`).text(item.name);
    $(`section:nth-child(${counter}) h3`).text(item.height);
    $(`section:nth-child(${counter}) p`).text(item.eye_color);
  });
}
...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.forEach( (ingredient) => {
    let removedNumbers = ingredient.slice(ingredient.indexOf(' ') + 1);
    let removedUnits = removedNumbers.slice(removedNumbers.indexOf(' ') + 1);
    result.push(removedUnits);
  })
  return result;
};
